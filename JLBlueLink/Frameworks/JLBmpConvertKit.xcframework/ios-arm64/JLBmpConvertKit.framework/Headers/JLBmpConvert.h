...
/// 图像转换代理
@protocol JLBmpConvertDelegate <NSObject>

-(void)jlBmpcovertResult:(JLImageConvertResult *)result;
...
/// 代理
@property (nonatomic, weak) id<JLBmpConvertDelegate> delegate;
...
///   - type: 转换类型
///   - inImage: 输入图片
+(void)covert:(JLBmpConvertType)type Image:(UIImage *)inImage;
...
///   - inFilePath: 输入文件
///   - outFilePath: 输出文件
+(void)covert:(JLBmpConvertType)type inFilePath:(NSString *_Nonnull)inFilePath outFilePath:(NSString * __nullable)outFilePath;
